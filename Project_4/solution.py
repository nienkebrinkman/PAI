...
class MLPActorCritic(nn.Module):
    ...
    def step(self, state):
        ...
        of chosen action.
        """
        # TODO: Implement the step function. DONE
        ...
        with torch.no_grad():
            ...
            _, log_prob = self.pi.forward(state, act=action)
...
class Agent:
    ...
    def train(self):
        ...
        for epoch in range(epochs):
            ...
            data = buf.get()
            # TODO: FRANZI -- multiply loss by discount factor before taking mean??
            # loss_pi = torch.tensor((buf.rewards_to_go * np.array(data['logp'])).mean())
            _, log_prob = self.ac.pi.forward(data['obs'], act=data['act'])
            loss_pi = -torch.sum(data['ret'].mul(log_prob))/len(ep_returns)
            ...
            for i in range(100):
                ...
                loss_v = criterion(values, data['ret']-values)
                if(i == 0):
                    print('loss_v:', loss_v)
                loss_v.backward()
                v_optimizer.step()
