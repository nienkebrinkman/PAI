...
def ece(probs, labels, n_bins=30):
    ...
    accuracies_per_bin = (
        np.bincount(probs_as_bin_num, weights=accuracies, minlength=n_bins) / total_per_bin
    )
...
class BayesianLayer(torch.nn.Module):
    ...
    def _kl_divergence(self, mu, logsigma):
        ...
        # (torch.exp(logsigma)**2 + (mu-self.prior_mu)**2)/(2*self.prior_sigma**2) - 0.5
        # return kl.sum()
        kl = torch.sum(
        np.log(self.prior_sigma)- logsigma + 1/(2 * self.prior_sigma ** 2)
...
class BayesNet(torch.nn.Module):
    ...
    def __init__(self, input_size, num_layers, width):
        super().__init__()
        input_layer = torch.nn.Sequential(BayesianLayer(input_size, width), nn.ReLU())
        hidden_layers = [
            nn.Sequential(BayesianLayer(width, width), nn.ReLU()) for _ in range(num_layers)
        ]
    ...
    def predict_class_probs(self, x, num_forward_passes=10):
        ...
        sum_x = torch.zeros((batch_size, 10))
    ...
    for i in pbar:
        for k, (batch_x, batch_y) in enumerate(train_loader):
            ...
            if type(model) == BayesNet:
                ...
                #     print(batch_y)
                #     print(y_pred)
                #     print('weights',list(model.net[1][0].parameters()))
                #     print(loss)
                #     print(model.loss)
                loss += model.loss *0.01
                # print(model.net[1][0].weight_mu[0,0])
                pass
...
def evaluate_model(model, model_type, test_loader, batch_size, extended_eval, private_test):
    ...
    if extended_eval:
        ...
        if not os.path.isfile(fmnist_path):
            fmnist_path = os.path.join(
                os.path.dirname(os.path.abspath(__file__)), "data/fashion/fmnist.npz"
            )
        ...
        for ax, idx in zip(axs, confidences_fmnist.argsort()[-10:]):
            ax.imshow(
                dataloader_fmnist.dataset.tensors[0][idx].numpy().reshape((28, 28)), cmap="gray"
            )
            ax.axis("off")
        fig.suptitle("Most confident predictions", size=20)
        ...
        dataset_labels = np.concatenate(
            [np.ones_like(confidences), np.zeros_like(confidences_fmnist)]
        )

        print(
            f"AUROC for MNIST vs. FashionMNIST OOD detection based on {model_type} confidence: "
            f"{roc_auc_score(dataset_labels, confidences_all):.3f}"
        )
        print(
            f"AUPRC for MNIST vs. FashionMNIST OOD detection based on {model_type} confidence: "
...
def main(test_loader=None, private_test=False):
    num_epochs = 170  # You might want to adjust this
    batch_size = 1280  # Try playing around with this
    print_interval = 100
    ...
    train_network(
        model, optimizer, train_loader, num_epochs=num_epochs, pbar_update_interval=print_interval
    )
    ...
    predictions = evaluate_model(
        model, model_type, test_loader, batch_size, extended_evaluation, private_test
    )
    return predictions
