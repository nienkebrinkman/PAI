...
class Model():
    ...
    def fit_model(self, train_x, train_y):
        ...
        fitted_model = Model_template(X, y, self.get_kernel("Matern-1/2")).double()
        self.fitted_model = fitted_model
        self.fitted_model = Model_template(X, y, self.get_kernel("Matern-1/2")).double()
        self.fitted_model = fitted_model
        actual_likelihood_noise = torch.tensor(data = [0.0017], dtype=torch.float64)
        self.fitted_model.likelihood.noise_covar.noise = actual_likelihood_noise
        actual_outputscale = torch.tensor(data = [0.1415], dtype=torch.float64)
        self.fitted_model.covar_module.outputscale = actual_outputscale
        actual_lengthscale = torch.tensor(data = [[2.6226]], dtype = torch.float64)
        self.fitted_model.covar_module.base_kernel.kernels[0].lengthscale = actual_lengthscale
...
def main():
    ...
    # cost = cost_function(eval_y.numpy(), prediction2)
    # print('cost ',cost)
